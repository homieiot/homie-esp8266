...
BootNormal::BootNormal()
    : Boot("normal"), _mqttReconnectTimer(MQTT_RECONNECT_INITIAL_INTERVAL, MQTT_RECONNECT_MAX_BACKOFF), _setupFunctionCalled(false), _mqttConnectNotified(false), _mqttDisconnectNotified(true), _otaOngoing(false), _flaggedForReboot(false), _mqttOfflineMessageId(0), _otaIsBase64(false), _otaBase64Pads(0), _otaSizeTotal(0), _otaSizeDone(0), _mqttTopic(nullptr), _mqttClientId(nullptr), _mqttWillTopic(nullptr), _mqttPayloadBuffer(nullptr), _mqttTopicLevels(nullptr), _mqttTopicLevelsCount(0)
{
...
void BootNormal::setup()
{
  ...
  if (Interface::get().led.enabled)
    Interface::get().getBlinker().start(LED_WIFI_DELAY);
  ...
  if (Interface::get().getConfig().get().mqtt.auth)
    Interface::get().getMqttClient().setCredentials(Interface::get().getConfig().get().mqtt.username, Interface::get().getConfig().get().mqtt.password);
...
void BootNormal::loop()
{
  ...
  if (_statsTimer.check())
  {
    ...
    if (signalPacketId != 0 && uptimePacketId != 0)
      _statsTimer.tick();
  }
...
void BootNormal::_wifiConnect()
{
  if (!Interface::get().disable)
  {
    ...
    { // on _validateConfigWifi there is a requirement for mask and gateway
      ...
      if (strcmp_P(Interface::get().getConfig().get().wifi.dns1, PSTR("")) != 0)
      {
        ...
        if ((strcmp_P(Interface::get().getConfig().get().wifi.dns2, PSTR("")) != 0))
        { // on _validateConfigWifi there is requirement that we need dns1 if we want to define dns2
...
void BootNormal::_mqttConnect()
{
  ...
  {
    if (Interface::get().led.enabled)
      Interface::get().getBlinker().start(LED_MQTT_DELAY);
...
void BootNormal::_advertise()
{
  ...
  case AdvertisementProgress::GlobalStep::PUB_HOMIE:
    ...
    if (packetId != 0)
      _advertisementProgress.globalStep = AdvertisementProgress::GlobalStep::PUB_MAC;
    break;
  case AdvertisementProgress::GlobalStep::PUB_MAC:
    ...
    if (packetId != 0)
      _advertisementProgress.globalStep = AdvertisementProgress::GlobalStep::PUB_NAME;
    break;
  case AdvertisementProgress::GlobalStep::PUB_NAME:
    ...
    if (packetId != 0)
      _advertisementProgress.globalStep = AdvertisementProgress::GlobalStep::PUB_LOCALIP;
    break;
  case AdvertisementProgress::GlobalStep::PUB_LOCALIP:
  {
    ...
    if (packetId != 0)
      _advertisementProgress.globalStep = AdvertisementProgress::GlobalStep::PUB_STATS_INTERVAL;
    break;
  }
  case AdvertisementProgress::GlobalStep::PUB_STATS_INTERVAL:
    ...
    if (packetId != 0)
      _advertisementProgress.globalStep = AdvertisementProgress::GlobalStep::PUB_FW_NAME;
    break;
  case AdvertisementProgress::GlobalStep::PUB_FW_NAME:
    ...
    if (packetId != 0)
      _advertisementProgress.globalStep = AdvertisementProgress::GlobalStep::PUB_FW_VERSION;
    break;
  case AdvertisementProgress::GlobalStep::PUB_FW_VERSION:
    ...
    if (packetId != 0)
      _advertisementProgress.globalStep = AdvertisementProgress::GlobalStep::PUB_FW_CHECKSUM;
    break;
  case AdvertisementProgress::GlobalStep::PUB_FW_CHECKSUM:
    ...
    if (packetId != 0)
      _advertisementProgress.globalStep = AdvertisementProgress::GlobalStep::PUB_IMPLEMENTATION;
    break;
  case AdvertisementProgress::GlobalStep::PUB_IMPLEMENTATION:
    ...
    if (packetId != 0)
      _advertisementProgress.globalStep = AdvertisementProgress::GlobalStep::PUB_IMPLEMENTATION_CONFIG;
    break;
  case AdvertisementProgress::GlobalStep::PUB_IMPLEMENTATION_CONFIG:
  {
    ...
    free(safeConfigFile);
    if (packetId != 0)
      _advertisementProgress.globalStep = AdvertisementProgress::GlobalStep::PUB_IMPLEMENTATION_VERSION;
    break;
  }
  case AdvertisementProgress::GlobalStep::PUB_IMPLEMENTATION_VERSION:
    ...
    if (packetId != 0)
      _advertisementProgress.globalStep = AdvertisementProgress::GlobalStep::PUB_IMPLEMENTATION_OTA_ENABLED;
    break;
  ...
  case AdvertisementProgress::GlobalStep::SUB_IMPLEMENTATION_OTA:
    ...
    if (packetId != 0)
      _advertisementProgress.globalStep = AdvertisementProgress::GlobalStep::SUB_IMPLEMENTATION_RESET;
    break;
  case AdvertisementProgress::GlobalStep::SUB_IMPLEMENTATION_RESET:
    ...
    if (packetId != 0)
      _advertisementProgress.globalStep = AdvertisementProgress::GlobalStep::SUB_IMPLEMENTATION_CONFIG_SET;
    break;
  ...
  case AdvertisementProgress::GlobalStep::SUB_BROADCAST:
  {
    ...
    if (packetId != 0)
      _advertisementProgress.globalStep = AdvertisementProgress::GlobalStep::PUB_ONLINE;
    break;
  }
...
{
  if (total == 0)
    return; // no empty message possible
...
bool HomieInternals::BootNormal::__fillPayloadBuffer(char *topic, char *payload, const AsyncMqttClientMessageProperties &properties, size_t len, size_t index, size_t total)
{
  ...
  if (_mqttPayloadBuffer == nullptr || index == 0)
    _mqttPayloadBuffer = std::unique_ptr<char[]>(new char[total + 1]);
...
{
  if (
      _mqttTopicLevelsCount == 5 && strcmp(_mqttTopicLevels.get()[0], Interface::get().getConfig().get().deviceId) == 0 && strcmp_P(_mqttTopicLevels.get()[1], PSTR("$implementation")) == 0 && strcmp_P(_mqttTopicLevels.get()[2], PSTR("ota")) == 0 && strcmp_P(_mqttTopicLevels.get()[3], PSTR("firmware")) == 0)
  {
    if (index == 0)
    {
...
{
  if (
      _mqttTopicLevelsCount == 2 && strcmp_P(_mqttTopicLevels.get()[0], PSTR("$broadcast")) == 0)
  {
...
{
  if (
      _mqttTopicLevelsCount == 3 && strcmp_P(_mqttTopicLevels.get()[1], PSTR("$implementation")) == 0 && strcmp_P(_mqttTopicLevels.get()[2], PSTR("reset")) == 0 && strcmp_P(_mqttPayloadBuffer.get(), PSTR("true")) == 0)
  {
...
{
  if (
      _mqttTopicLevelsCount == 4 && strcmp_P(_mqttTopicLevels.get()[1], PSTR("$implementation")) == 0 && strcmp_P(_mqttTopicLevels.get()[2], PSTR("config")) == 0 && strcmp_P(_mqttTopicLevels.get()[3], PSTR("set")) == 0)
  {
