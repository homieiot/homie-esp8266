...
```c++
HomieSetting<unsigned long> percentageSetting("percentage", "A sample percentage");  // name, description

void setup() {
  rgbSetting.setDefaultValue(50).setValidator([] (unsigned long candidate) {
...
See the `CustomSettings` example for a concrete use case.
...
## Know if the device is in normal mode
